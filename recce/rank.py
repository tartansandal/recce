...
# The pruning ladder, tried in order until the map fits its budget.
...
class Node:
    ...
    def line_count(self) -> int:
        return 1 + sum(child.line_count() for child in self.children)
        ...
        for depth_cap in _DEPTH_LADDER:
            for external_depth in (99, 2):
                ...
                if sum(n.line_count() for n in attempt) <= max_lines:
                    return attempt
                # Rung five is free once the trees exist: roots are already in
                # score order, so taking the prefix that fits drops the least
                # interesting flows without rebuilding anything. Rebuilding per
                # root count is what made this quadratic, and on a package the
                # size of asyncio it cost twenty-odd seconds.
